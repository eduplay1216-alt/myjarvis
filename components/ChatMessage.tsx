...
export const ChatMessage: React.FC<ChatMessageProps> = ({ message, isLoading }) => {
  ...
  const bubbleClasses = isUser 
    ? 'bg-blue-600 rounded-lg p-3 shadow-md max-w-3/5 sm:max-w-2/3 lg:max-w-lg' 
    : 'bg-gray-800 rounded-lg p-3 shadow-md min-h-[44px] flex items-center max-w-3/5 sm:max-w-2/3 lg:max-w-lg';
  
  const iconContainerClasses = `w-10 h-10 flex-shrink-0 rounded-full flex items-center justify-center border-2 ${isUser ? 'bg-gray-700 border-gray-500' : 'bg-blue-500/20 border-blue-400'}`;
    ...
    <>
      ...
      <div className={bubbleClasses}>
        {isLoading && !message.text ? (
          ...
          <p className="text-gray-200 whitespace-pre-wrap break-words min-w-0">
